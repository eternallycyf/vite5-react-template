...
import { ComponentSize } from '#/enum'
import { useSettingActions, useSettings } from '@/store/settingStore'
import { useThemeToken } from '@/hooks/theme'
import { colorPrimarys } from '@/theme/antd-theme'
import { FullScreen, LocalePicker, ThemeModeBtn } from '@/layouts/core'
import { t } from '@/locales/i18n'
import useLocale from '@/locales/useLocale'

function SettingButton() {
  const [drawerOpen, setDrawerOpen] = useState<boolean>(false)
  const { colorPrimary, colorTextSecondary, colorTextTertiary, colorBgContainer }
    = useThemeToken()
  const { locale } = useLocale()

  const settings = useSettings()
  const { themeColorPresets, componentSize, themeStretch, breadCrumb }
    = settings
